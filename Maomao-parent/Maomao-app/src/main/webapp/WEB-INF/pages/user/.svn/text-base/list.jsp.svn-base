	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<ul class="pagination">
				    <li><a href="${PRO_PATH}/userList?pn=1">首页</a></li>
				    
				    <c:if test="${pageInfo.hasPreviousPage}">
					<li><a href="${PRO_PATH}/userList?pn=${pageInfo.pageNum-1}" aria-label="Previous"> <span
					...
					<c:forEach items="${pageInfo.navigatepageNums}" var="apageNum">
					 ...
					 <c:if test="${apageNum!=pageInfo.pageNum}">
					  <li><a href="${PRO_PATH}/userList?pn=${apageNum}">${apageNum}</a></li>
					  </c:if>
					</c:forEach>
					
					<c:if test="${pageInfo.hasNextPage}">
					<li><a href="${PRO_PATH}/userList?pn=${pageInfo.pageNum+1}" aria-label="Next"> <span
							aria-hidden="true">&raquo;</span>
					</a></li>
					</c:if>
					
					 <li><a href="${PRO_PATH}/userList?pn=${pageInfo.pages}">末页</a></li>
				</ul>
				</nav>
			</div>
		</div>
